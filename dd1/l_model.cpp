...
md_t * L_CreateModelFromResourceName( char *name )
{
	...
	md->md_res = li.GI_ResourceSearch( tt );
	if ( !md->md_res )
	...
	for ( ; ( coordsys = SearchGetNextClass( &iter ) ) ; )
	{
		...
		if ( 0 )
		{
			g_resource_t	*submd_res;

			submd_res = li.GI_ResourceSearch( "submd.testobj.box" );
			__chkptr( submd_res );
			MD_CoordsysInsertSubmodel( cs, submd_res->res_cache );
		}
	...
	for ( ; ( cs1 = U_ListIterNext( &list1_iter ) ) ; )
	{
		...
		for ( ; ( cs2 = U_ListIterNext( &list2_iter ) ) ; )
		{
	...
	for ( ; ( sm = U_ListIterNext( &list1_iter ) ) ; )
	{
		...
		sm->res_submd = li.GI_ResourceSearch( pair1->value );
		if ( !sm->res_submd )
			__error( "can't find resource '%s'\n", pair1->value );
		...
		sm->res_gltex = li.GI_ResourceSearch( pair1->value );
		if ( !sm->res_gltex )
			__error( "can't find resource '%s'\n", pair1->value );
		...
		// search coordsys
		U_ListIterInit( &list2_iter, &md->coordsys_list );
		for ( ; ( cs1 = U_ListIterNext( &list2_iter ) ) ; )
		{
...
void L_ModelSetMoveByName( md_t *md, char *name )
{
	...
	for ( ; ( cs = U_ListIterNext( &iter ) ) ; )
	{
		...
		// search move name
		move = U_MapSearch( &md_cache->moves_map, &tmp );
		if ( !move )
			continue;
